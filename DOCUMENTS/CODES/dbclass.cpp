...
public:
    ...
    std::string Call_requestMate(std::string user_no)
    {
        std::string result = "";
        ...
        try
        {
            ...
            sql::PreparedStatement *stmnt1 = m_connection->prepareStatement("SELECT NO, NICK FROM USER WHERE USER.NO IN(SELECT SENDID FROM MATE WHERE RECVID = ? AND STATUS = -1)");
            stmnt1->setInt(1, std::stoi(user_no));
